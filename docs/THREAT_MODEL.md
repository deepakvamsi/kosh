...
| T2 | Offline brute-force of the master password | Info disclosure | Argon2id with memory-hard parameters + per-vault random salt makes guessing expensive. The cost is calibrated to the machine at vault creation (64–512 MiB, 3–10 passes, targeting ~750 ms per attempt) rather than fixed at a floor chosen for the slowest supported hardware, and can be raised later via `RekeyKDF` without re-encrypting any secret. This remains the *only* barrier once an attacker holds the database file, so it is deliberately the one parameter the user can increase. |
...
| T4 | Audit log tampering / deletion of evidence | Tampering / Repudiation | Append-only, hash-chained records; each entry commits to the previous hash. Records written while unlocked additionally carry an HMAC under a DEK-derived subkey, and the chain head is anchored in `vault_meta` under the same key, so an attacker with database write access cannot rewrite history or truncate the tail without the master password. Records written while LOCKED (failed unlock, lockout) cannot be keyed — no DEK exists — and are chain-only; a trailing run of such records is the residual gap. `VerifyChainKeyed` performs the full check. |
